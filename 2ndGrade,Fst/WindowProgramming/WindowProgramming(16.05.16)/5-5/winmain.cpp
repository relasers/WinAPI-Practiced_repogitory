...
static bool imgbackobst = false;
static int setimg = IMG::img1;
static float size = 1;
static int speed = 3;
static Player player;
static int counter = 0;
static bool istext = false;
static bool isobsta = false;
HINSTANCE hInst;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	static RECT fortext = {0,0,30,100};
	static int textvec = 1;
	...
	static wchar_t temptext[128] = TEXT("나는 텍스트를 출력하고 이따");
	...
	case WM_CREATE:
		...
		Bit_Player2 = (HBITMAP)LoadBitmap(hInst, // hinstance 인수는 맨위에 선언한 HINSTANCE 값으로..
	...
	case WM_TIMER:
		...
		case WT_UPDATE:
			...
			if (0 < counter) {
				...
				if (player.dir == player_dir::left)
					OffsetRect(&player.pos, -speed, 0);
				else
					OffsetRect(&player.pos, speed, 0);
			...
			OffsetRect(&fortext,5*textvec,0);
			if (fortext.left <= 0 || fortext.right >= rectView.right) {
				textvec *= -1;
				OffsetRect(&fortext, 5 * textvec, 0);
			}
	...
	case WM_PAINT: //윈도우 다시 그려줘야 할 때
		...
		SelectObject(LayDC, Lay);
		{
			...
			if(isobsta)
				TransparentBlt(LayDC, obstacle.left-48, obstacle.top, 64, 64
					, memdc, obstacleframe * 64, 0, 64, 64, RGB(255, 255, 255)); // 장애물을 그린다
			...
			if(setimg == IMG::img1){
				...
				SelectObject(memdc, Bit_Player2);
			}
				

			TransparentBlt(LayDC, player.pos.left, player.pos.top, 32 * size, 48 * size
			...
			if (istext)
				DrawText(LayDC,temptext,-1,&fortext,DT_CENTER|DT_NOCLIP);
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	static int Check[2], Radio1, Radio2;
	switch (iMsg)
	{
	case WM_INITDIALOG:
		CheckRadioButton(hDlg,
			IDC_RADIO1_1, IDC_RADIO1_2, IDC_RADIO1_1);
		// 시작 버튼, 끝 버튼, 미리 체크할 버튼
		CheckRadioButton(hDlg,
			IDC_RADIO2_EXTEND, IDC_RADIO2_DEFAULT, IDC_RADIO2_DEFAULT);
		return 0;
	case WM_COMMAND:
		...
		case IDC_RADIO1_2:
			...
			setimg = IMG::img2;
			break;
		case IDC_RADIO2_EXTEND:
			Radio2 = 0;
			size = 2.0;
			break;
		case IDC_RADIO2_SHRINK:
			Radio2 = 1;
			size = 0.5;
			break;
		case IDC_RADIO2_DEFAULT:
			Radio2 = 2;
			size = 1.0;
			break;
		case IDCANCEL:
