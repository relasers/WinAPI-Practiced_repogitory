...
LRESULT CALLBACK MDIChildProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
...

		{
			...
			ReadFile(hFile, data[ID].string[0], size, &size, NULL);
			CloseHandle(hFile);
...

	{
		...
		if (GetSaveFileName(&OFN) != 0) {
			wsprintf(str, L"%s 파일에 저장하시겠습니까 ?",
				OFN.lpstrFile);
			MessageBox(hWnd, str, L"저장하기 선택", MB_OK);
		}
			HANDLE hFile;
			hFile = CreateFile(lpstrFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, 0);
